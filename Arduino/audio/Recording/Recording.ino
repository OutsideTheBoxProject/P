...
/*
 * Notes: changes to pcmConfig.h (library)
 * increased buffsize to 254
 * uncommented ENABLE_RECORDING
 * uncommented BLOCK_COUNT 10000L
 */
...
//  pinMode(7,OUTPUT);  //Pin pairs: 9,10 Mega: 5-2,6-7,11-12,46-45
...
void loop() {
    ...
    if(Serial.available()){                          //Send commands over serial to play
      switch(Serial.read()){
        case 'r': audio.startRecording("test.wav",16000,A0); Serial.println(" recording");  break;    //Record at 16khz sample rate on pin A0
        ...
        case 's': audio.stopRecording("test.wav"); Serial.println(" stopping"); break;              //Stop recording
        case 'p': audio.play("test.wav"); Serial.println(" playing"); break;                       //Play the recording 
